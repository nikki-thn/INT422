...
namespace Web_app_project_template_v11.Controllers
{
    public class Manager
    {
        ...
        //public PlaylistBase PlaylistGetByIdWithTracks(int id)
        //{
        //    //Attempt to fetch the object
        ...
        //Use AutoMapper to map objects, source to target
        public IEnumerable<PlaylistWithTracksDetails> PlaylistGetAllWithTracks()
        {
            return mapper.Map<IEnumerable<Playlist>, IEnumerable<PlaylistWithTracksDetails>>
                (ds.Playlists.Include("Tracks").OrderBy(p => p.Name));
        }

        public PlaylistWithTracksDetails PlaylistGetByIdWithTracks(int id)
        {
            //Attempt to fetch the object
            var o = ds.Playlists.Include("Tracks").SingleOrDefault(p => p.PlaylistId == id);

            //Return the result, or null if not found
            return (o == null) ? null : mapper.Map<Playlist, PlaylistWithTracksDetails>(o);
        }
